...
$stmt = db()->prepare("
    ...
    FROM (
        ...
        UNION ALL
        SELECT COALESCE(orders.order_type, 'Tempahan') AS sale_type, orders.total, CASE WHEN orders.order_type = 'Frozen' THEN orders.qty * COALESCE(frozen_stock.pieces_per_unit, 1) ELSE orders.qty END AS qty
        FROM orders
        LEFT JOIN frozen_stock ON frozen_stock.id = orders.frozen_stock_id
        WHERE orders.status = 'Completed' AND orders.pickup_date BETWEEN ? AND ?
        UNION ALL
